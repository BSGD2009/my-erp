...
export function MaterialTypesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await api.get<MaterialType[]>('/protected/material-types');
      setRows(res);
  ...
  function openEdit(r: MaterialType) {
  ...
  async function save() {
    ...
    try {
      ...
      if (editId) await api.put(`/protected/material-types/${editId}`, body);
      else        await api.post('/protected/material-types', body);
  ...
  async function deleteType(id: number, e: React.MouseEvent) {
    e.stopPropagation();
    if (!confirm('Deactivate this material type?')) return;
    try {
      await api.delete(`/protected/material-types/${id}`);
      load();
    } catch (err: any) {
  ...
  return (
    <Layout>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem', flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h1 style={{ fontSize: '1.5rem', fontWeight: 700, margin: 0, letterSpacing: '-0.02em' }}>Material Types</h1>
      ...
      <div style={{ ...cardStyle, overflow: 'hidden', maxWidth: 640 }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            ...
            {!loading && rows.length === 0 && <tr><td colSpan={4} style={{ padding: '3rem', textAlign: 'center', color: c.textMuted }}>No material types found.</td></tr>}
            {!loading && rows.map(r => (
      ...
      <Drawer open={drawerOpen} onClose={() => setDrawerOpen(false)} title={editId ? 'Edit Material Type' : 'New Material Type'}>
        ...
        <Field label="Type Key *"><input style={inputStyle} value={f.typeKey} onChange={set('typeKey')} placeholder="BOARD" /></Field>
        <Field label="Type Name *"><input style={inputStyle} value={f.typeName} onChange={set('typeName')} placeholder="Corrugated Board" /></Field>
